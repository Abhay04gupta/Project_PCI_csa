...
def process_audio():
    ...
    try:
        ...
        if "help" in question.lower():
        ...
        elif "help" not in question.lower() and use_test_folder == False:
            # LLM processing
            # model_cpt = "tiiuae/falcon-7b-instruct"
            ...
            answer="Your result for communication test is on the screen"
            ...
            storage_folder = app.config['UPLOAD_FOLDER_GEN']
            generated_folder = app.config['GENERATED_FOLDER_GEN']
            use_test_folder = False
            return jsonify({"error": "Unexpected condition"}), 400
...
def analyze_audio(y, sr):
    ...
    stressed_syllables_tag = "Narural" if stressed_syllables_count < 70 else "Dramatic"
    
    falling_intonation_tag = "Confident Enough" if falling_intonation else "Need Confidence"
    ...
    fluency_tag = "Good Fluency" if not fluency_improvement_needed else "Bad Fluency"
...
# def detect_silent_sections(y, threshold_db=-55, min_silence_duration=0.3, tolerance_db=3,tolerance_duration=0.05):
...
#     return {
#         "total_duration":float(total_duration),
#         "mean_pitch": {"value":float(mean_pitch),"tag":"good"},
